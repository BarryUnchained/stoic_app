...
class _SetUsernameScreenState extends State<SetUsernameScreen> {
  ...
  Future<void> _save() async {
    ...
    if (name.length > 20) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('昵称最多20个字')));
      return;
    }
    setState(() => _saving = true);
    try {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 32),
            TextField(
              controller: _controller, maxLength: 20, autofocus: true,
              decoration: const InputDecoration(hintText: '输入昵称', border: OutlineInputBorder(), counterText: ''),
              onSubmitted: (_) => _save(),
            ),
            const SizedBox(height: 24),
            SizedBox(
              width: double.infinity, height: 48,
              child: ElevatedButton(onPressed: _saving ? null : _save, child: _saving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('确认')),
            ),
            const SizedBox(height: 12),
